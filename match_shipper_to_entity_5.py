...
from jurisdiction_neighborhood import get_regional_jurisdictions_by_country, get_iso_code_by_country
...
def calculate_match_score(trademo_tokens: set[str], entity_tokens: set[str], 
                         shipper_jurisdiction: str, entity_jurisdiction: str,
                         regional_jurisdictions: list[str]) -> float:
    """
    ...
    shipper_code = shipper_jurisdiction.upper() if len(shipper_jurisdiction) == 2 else get_iso_code_by_country(shipper_jurisdiction)
    ...
    elif entity_main_jurisdiction.upper() in [j.upper() for j in regional_jurisdictions]:
        jurisdiction_score = 0.3

    # Calculate final weighted score
    final_score = (name_score * 0.7) + (jurisdiction_score * 0.3)

    return final_score

def find_matches(trademo_tokens: set[str], shipping_country: str, 
    ...
    shipper_code = shipping_country.upper() if len(shipping_country) == 2 else get_iso_code_by_country(shipping_country)

    # Search for potential matches
    for entity in entities.find(query).limit(20).hint('tokenized_name_1'):
        ...
        # Calculate final weighted score
        score = (name_score * 0.7) + (jurisdiction_score * 0.3)
